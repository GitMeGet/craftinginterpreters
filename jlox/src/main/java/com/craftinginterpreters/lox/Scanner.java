...
class Scanner {
  ...
  private void scanToken() {
    ...
    switch (c) {
      ...
      case '\r':
      case '\t':
        // ignore them except newline
        break;
      case '\n':
        line++;
        break;
      // identifier, numbers, invalids
      default:
        if (isDigit(c)) {
          number();
        } else if (isAlpha(c)) {
  ...
  /* conditional advance()
   * only consumes the current character if it’s what we’re looking for.
   */
  ...
  // like advance(), but doesn’t consume the character. This is called lookahead.
  private char peek() {
    if (isAtEnd()) return '\0';
